...
def long_words(words):
    ...
    # iterates over each word in word and returns word if length of word is 
    # greater than 4.
...
def n_long_words(words, n):
    ...
    # iterates over each word in words if length of word is greater than n.
...
def smallest_int(numbers):
    ...
    **DO NOT USE** the built-in function `min()`!
    ...
    # if list is not empty, sorts the list, and returns smallest integer at 
    # index 0.
    else:
        numbers.sort()
        return numbers[0]
...
def average(numbers):
    ...
    # there are two solutions to handle empty input lists; try & except and if conditional:

    # try:
    #     for num in numbers:
    #         sum_num += num

    #     return sum_num / float(len(numbers))

    # except ZeroDivisionError:
    #     print "Error.  Unable to compute average of empty list."

    if numbers == []:
        print "Unable to compute average of an empty list."
...
def duplicates(items):
    ...
    return list(sorted(duplicate_items))
    ...
    **DO NOT** use the `list.index()` method.
    ...
    for word in words:
        if letter in word:
            ...
            letter_indices_list.append(index)
